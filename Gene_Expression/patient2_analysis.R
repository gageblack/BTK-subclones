...
## Remove non-B Cells from post-treatment sample
notBcells = c("GCAACGCTGTTCGGGC-1","TTTCATGTGGATCAAA-1","CTCTGTGTGTCCACAA-1","CGGTAGCTGTGGTTGT-1",
              "CTCGATGCTAAGACAT-1", "CGACATTGAGACAGTG-1", "ATCGGTTGAGAATGTA-1", "AGTGTGTTGACTCCCT-1")
seurat_df_pre_allB = subset(seurat_df_pre, cells = notBcells, invert = TRUE)

# Run SCTransform on both samples.
seurat_df_pre_sct <- SCTransform(seurat_df_pre_allB, method = "glmGamPoi", vst.flavor = "v2")
...
points(umap_pre_coordinate[rownames(umap_pre_coordinate) %in% sc0,][,2],umap_pre_coordinate[rownames(umap_pre_coordinate) %in% sc0,][,3], pch=16, cex=0.6,col="green")
points(umap_pre_coordinate[rownames(umap_pre_coordinate) %in% sc1,][,2],umap_pre_coordinate[rownames(umap_pre_coordinate) %in% sc1,][,3], pch=16, cex=0.6,col="red")
...
points(umap_post_coordinate[rownames(umap_post_coordinate) %in% sc2,][,2],umap_post_coordinate[rownames(umap_post_coordinate) %in% sc2,][,3], pch=16, cex=0.5,col="blue")
points(umap_post_coordinate[rownames(umap_post_coordinate) %in% sc1,][,2],umap_post_coordinate[rownames(umap_post_coordinate) %in% sc1,][,3], pch=16, cex=0.5,col="red")
points(umap_post_coordinate[rownames(umap_post_coordinate) %in% sc3,][,2],umap_post_coordinate[rownames(umap_post_coordinate) %in% sc3,][,3], pch=16, cex=0.5,col="black")
points(umap_post_coordinate[rownames(umap_post_coordinate) %in% sc0,][,2],umap_post_coordinate[rownames(umap_post_coordinate) %in% sc0,][,3], pch=16, cex=0.5,col="green") ## Only 1 shows up.
dev.off()
